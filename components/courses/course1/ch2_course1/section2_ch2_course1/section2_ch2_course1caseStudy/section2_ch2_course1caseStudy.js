...
import styles from './section2_ch2_course1caseStudy.module.css';
...
const question = {
  question: "What round is Startup A likely raising? ",
  answers: ["Pre-Seed", "Seed", "Series A", "Series B"],
  correctAnswer: "Series A"
}
...
function Section1_1CaseStudy(props) {
  ...
  const defaultSubmittedCorrectAnswer = student.courseProgress?.course_1?.chapter_2?.section_2?.q_1?.complete;
  ...
  return (
    <div id={styles.rightCourseColumnContainer}>
      ...
      <p id={styles.caseStudyParagraph}>
        Bessemer Venture Partners, a VC firm with more than $20 billion of assets under management, is looking for new investments. One analyst sources 2 new startups:
      </p>
      <br></br>
      <p id={styles.caseStudyParagraph}>
        Startup A: $15 million valuation medtech startup      </p>
      <br></br>
      <p id={styles.caseStudyParagraph}>
        Startup B: $190 million valuation gaming startup      </p>
...

      {
        submittedCorrectAnswer ? (
          <div id={styles.continueButtonContainer}>
            <White_Button button_title="Continue" onClick={() => { router.push("/courses/course1/ch3_course1/section1_ch3_course1"); }} /> </div>
        ) : (
          <VStack id={styles.continueButtonContainer}>
            <White_Button button_title="Submit" onClick={() => {
              const isCorrect = selectedAnswer == question.correctAnswer;
              submitAnswer(user.token, "course_1", "chapter_2", "section_2", "q_1", isCorrect).then(() => refetchStudent());
